...
def rotation_3d(omega, theta):
    ...
    R = I + (omega_hat/omega_mag)*sin(omega_mag * theta) + (np.matmul(omega_hat,omega_hat)/omega_mag**2)*(1 - cos(omega_mag * theta))
...
def hat_3d(xi):
    ...
    # YOUR CODE HERE
    skew_mat = skew_3d(xi[3:])
    v = xi[:3].reshape((3,1))
    xi_hat = np.concatenate((skew_mat, v), axis=1)
    xi_hat = np.concatenate((xi_hat, np.zeros((1, 4))), axis=0)
    return xi_hat
...
def homog_3d(xi, theta):
    ...
    if xi[3]==0 and xi[4]==0 and xi[5]==0:
        v_theta = theta*xi[:3]
        I = np.identity(3)
        I_new = np.concatenate((I, v_theta.reshape((3, 1))), axis=1)
        bottom = np.zeros((1,4))
        bottom[0][3] = 1
        g = np.concatenate((I_new, bottom), axis=0)
    else:
        omega = np.array(xi[3:])
        v = np.array(xi[:3])
        exp = rotation_3d(omega, theta)

        ## big term calcs ##
        omag = np.linalg.norm(omega, 2)
        I = np.identity(3)
        t1 = I  - exp
        t2 = np.dot(skew_3d(xi[3:]),v)
        t1 = np.dot(t1, t2
        )
        t2 = np.dot(omega, np.dot(omega.T, v)) * theta

        big_term = (t1 + t2) / omag**2
        g = np.concatenate((exp, big_term.reshape((3, 1))), axis = 1)
        bottom = np.zeros((1,4))
        bottom[0][3] = 1
...
def prod_exp(xi, theta):
    ...
    # YOUR CODE HERE
    g = np.identity(4)
    disp_count = len(theta)
    for i in range(0, disp_count):
        twist_vec = xi[:, i]
        theta_i = theta[i]
        g_curr = homog_3d(twist_vec, theta_i)
        g = np.dot(g, g_curr)
    return g
