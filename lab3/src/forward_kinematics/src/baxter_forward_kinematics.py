...
def baxter_forward_kinematics_from_angles(joint_angles):
    ...
    qs0 = qs[0:3, 7].reshape(3, 1)
    g_st0 = np.vstack((np.hstack((R, qs0)), np.array([0, 0, 0, 1])))

    xi_all = np.ndarray((6,7))
    counter = 0
    for q, w in zip(qs.T, ws.T):
        xi = np.vstack((np.cross(-w, q), w)).reshape(1,6)
        xi_all[0:6,counter] = xi
        counter += 1


    g_st = kfs.prod_exp(xi_all, joint_angles).dot(g_st0)
...
def baxter_forward_kinematics_from_joint_state(joint_state):
    ...
    p_arr = joint_state.position[2:9]
    # print(p_arr)
    angles = np.array([p_arr[2], p_arr[3], p_arr[0], p_arr[1], p_arr[4], p_arr[5], p_arr[6]])
...
def main():
    ...
    baxter_forward_kinematics_from_joint_state(joint_state)
    # joint_angles = [0, 0, 0, 0, 0, 0, 0]
    # baxter_forward_kinematics_from_angles(joint_angles)
